...
function handleClick(clickedButtonEvent) {
    ...
    if (this.innerHTML == "Подробнее") { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
    //     // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card);
    } else if (this.innerHTML == "Отмена") { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function showMoreText(card) {
    ...
    let text = "In publishing and graphic design, Lorem ipsum is a placeholder text commonly used";
